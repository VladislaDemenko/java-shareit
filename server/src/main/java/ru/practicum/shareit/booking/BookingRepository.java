...
public interface BookingRepository extends JpaRepository<Booking, Long> {
    ...
    @Query("SELECT b FROM Booking b WHERE b.itemId IN :itemIds AND b.status = 'APPROVED' AND b.start < :now ORDER BY b.start DESC")
    List<Booking> findLastBookingsForItems(@Param("itemIds") List<Long> itemIds, @Param("now") LocalDateTime now);

    @Query("SELECT b FROM Booking b WHERE b.itemId IN :itemIds AND b.status = 'APPROVED' AND b.start > :now ORDER BY b.start ASC")
